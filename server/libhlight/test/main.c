...
int main(void)
{
	...
	puts("Starting Home Light test...");
	puts("--------------------------------------------");

	if (!hlight_set_log("hlight.log")) {
		exit_fail("Setting log path.");
		return -1;
	}
	puts("[OK] Setting log path.");

	if (hlight_load_configs("/etc/smarthome/hlight.conf") != HL_CFG_OK) {
