 ...
 */
uint8_t hl_configs_load(const char *filename)
{
    FILE *file;

    file = fopen(filename, "r");
    if (file == NULL)
        return HL_CFG_FILE_NOT_FOUND;

    if (!configs_read_string(file, hl_cfg.ssc.ip, 15)) {
        fclose(file);
        return HL_CFG_PARSE_ERR;
    }
    if (!configs_read_unsigned(file, &hl_cfg.ssc.port)) {
        fclose(file);
        return HL_CFG_PARSE_ERR;
    }
    fclose(file);
...
const struct server_cfg *hl_configs_get_server(void)
{
    return &hl_cfg.ssc;
}