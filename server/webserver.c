...
static void call_security_module(const char *command, struct tcp_client *client)
{
	char func[50];
	char answ[255];
	...
	if (!strcmp(command, "get_status")) {
		...
		if (!house_security_get_status(&stat)) {
...
static void new_session(struct tcp_client *client, void *data)
{
	...
	if (!strcmp(module, "/security")) {
		call_security_module(command, client);
		return;
	}
}
